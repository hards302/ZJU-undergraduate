...
from training.train_spiking_ddpg.sddpg_networks import ActorNetSpiking, ActorNetSpikingwithNCP, ActorNetSpikingwithRSTDP
...
def load_test_actor_snn_ncp_network(weight_dir, bias_dir, device, batch_window=50,
                                state_num=24, action_num=2, dim=(256, 256)):
    """
    Load actor snn network for testing
    :param weight_dir: directory of numpy weights
    :param bias_dir: directory of numpy bias
    ...
    :return: actor_net
    """
    actor_net = ActorNetSpikingwithNCP(state_num, action_num, device,
                                batch_window=batch_window,
                                inter_num=dim[0],
                                command_num=dim[1])
    weights = pickle.load(open(weight_dir, 'rb'))
    bias = pickle.load(open(bias_dir, 'rb'))
    actor_net.w1 = nn.Parameter(torch.from_numpy(weights[0]))
    actor_net.w2 = nn.Parameter(torch.from_numpy(weights[1]))
    actor_net.w3 = nn.Parameter(torch.from_numpy(weights[2]))
    actor_net.w4 = nn.Parameter(torch.from_numpy(weights[3]))
    actor_net.b1 = nn.Parameter(torch.from_numpy(bias[0]))
    actor_net.b2 = nn.Parameter(torch.from_numpy(bias[1]))
    actor_net.b3 = nn.Parameter(torch.from_numpy(bias[2]))
    actor_net.b4 = nn.Parameter(torch.from_numpy(bias[3]))
    return actor_net

def load_test_actor_snn_rstdp_network(weight_dir, bias_dir, device, batch_window=50,
                                state_num=24, action_num=2, dim=(256, 256, 256)):
    """
    Load actor snn network for testing
    :param weight_dir: directory of numpy weights
    ...
    :return: actor_net
    """
    actor_net = ActorNetSpikingwithRSTDP(state_num, action_num, device,
                                batch_window=batch_window,
                                hidden1=dim[0],
                                hidden2=dim[1],
                                hidden3=dim[2])
