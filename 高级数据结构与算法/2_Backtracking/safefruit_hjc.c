...
// greedy pruning is possibly wrong
const int NUM=105;
...
// note that the graph is dense, try using greedy to get approx size for pruning
...
*/
void Solve(int* state, int cpos, int choice, int size, Book* book,int* rfind,int**tips,int* prices, int approx);
...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    // for testing
    freopen("./Backtracking/test/test.txt","r",stdin);
    ...
    // INPUT END
    // using greedy way to find approx
    int approx=0;
    state[0]=1;approx++;
    for(int i=1; i<M;++i)
    {
    ...
    // set state[0]=0;
    Solve(state,cpos,0,M,&book,rfind,(int**)tips,prices,approx);
    ...
    #endif
    Solve(state,cpos,1,M,&book,rfind,(int**)tips,prices,approx);
...
void Solve(int* state, int cpos, int choice, int size, Book* book,int* rfind,int**tips,int* prices,int approx)
{
    ...
    */
    ...
    Solve(state,cpos,0,size,book,rfind,tips,prices,approx);
    /*
        UPDATE rfind
    ...
    */
    ...
    if(rfind[cpos] && count1+rfind[cpos+1]<approx) return;
    ...
    */
    ...
    Solve(state,cpos,1,size,book,rfind,tips,prices,approx);
}
