...
from FedLearn import FederatedSchemewithRNN
from Data.Datasets import get_mnist_test
from Examples.MNIST.Model import MNIST_RNN
from Examples.MNIST.GenClients import get_iid_mnist, get_non_iid_mnist
import matplotlib.pyplot as plt

mnist_test = get_mnist_test()

clients = get_iid_mnist(100)  # Each client get 500 samples
# clients = get_non_iid_mnist(2, 50)   # use it to change to data to non-iid. and remember change "FederatedSchemewithRNN" in FedLearn too.
model = MNIST_RNN

#--------------do research on C,B,E just need to change it and redo code below----------
n_clients = 10  # C = n_clients/100
client_batch_size = 10  # B
n_client_batches = 1  # E

fed_scheme = FederatedSchemewithRNN(clients, lambda: model(), nn.MSELoss())
...
for i in range(9001):
    ...
    if i % 100 == 0:
        ...
        test_xs = test_xs.view(1000,28,28)
    ...
    fed_scheme.fed_avg_one_step(n_clients, client_batch_size, n_client_batches, 1.47)
