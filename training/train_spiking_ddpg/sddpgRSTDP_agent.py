...
from training.train_spiking_ddpg.sddpg_networks import ActorNetSpikingwithRSTDP


class RSTDPAgentSpiking:
    """
    ...
    def __init__(self,
        ...
        """
        self.actor_net = ActorNetSpikingwithRSTDP(self.spike_state_num, self.action_num, self.device,
        ...
        self.target_actor_net = ActorNetSpikingwithRSTDP(self.spike_state_num, self.action_num, self.device,
                                                hidden1=actor_net_dim[0],
                                                hidden2=actor_net_dim[1],
                                                hidden3=actor_net_dim[2],
                                                batch_window=self.batch_window)
    ...
    def save(self, save_dir, episode, run_name):
        ...
        l1_weights = self.actor_net.w1.data.numpy()
        l1_bias = self.actor_net.b1.data.numpy()
        l2_weights = self.actor_net.w2.data.numpy()
        l2_bias = self.actor_net.b2.data.numpy()
        l3_weights = self.actor_net.w3.data.numpy()
        l3_bias = self.actor_net.b3.data.numpy()
        l4_weights = self.actor_net.w4.data.numpy()
        l4_bias = self.actor_net.b4.data.numpy()
        pickle.dump([l1_weights, l2_weights, l3_weights, l4_weights],
                    open(save_dir + '/' + run_name + '_rstdp_weights_s' + str(episode) + '.p', 'wb+'))
        pickle.dump([l1_bias, l2_bias, l3_bias, l4_bias],
                    open(save_dir + '/' + run_name + '_rstdp_bias_s' + str(episode) + '.p', 'wb+'))
    ...
    def _update(self, reward):
        self.actor_net._update(reward)
        self.target_actor_net._update(reward)