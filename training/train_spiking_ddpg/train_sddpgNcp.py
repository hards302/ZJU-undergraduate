...
sys.path.append('../../')
sys.path.append('..')
from training.train_spiking_ddpg.sddpgNcp_agent import NcpAgentSpiking
...
def train_sddpg(run_name="SNN_R1", exp_name="Rand_R1", episode_num=(100, 200, 300, 400),
    ...
    agent = NcpAgentSpiking(state_num, action_num, spike_state_num,
    ...
    while True:
        ...
        for ita in range(ita_per_episode):
            ...
            if len(agent.memory) > batch_size:
                ...
                tb_writer.add_scalar('Spike-DDPG-NCP/actor_loss', actor_loss_value, overall_steps)
                tb_writer.add_scalar('Spike-DDPG-NCP/critic_loss', critic_loss_value, overall_steps)
            ita_time_end = time.time()
            tb_writer.add_scalar('Spike-DDPG-NCP/ita_time', ita_time_end - ita_time_start, overall_steps)
            tb_writer.add_scalar('Spike-DDPG-NCP/action_epsilon', agent.epsilon, overall_steps)
            tb_writer.add_scalar('Spike-DDPG-NCP/raw_left_wheel_output', raw_snn_action[0], overall_steps)
            tb_writer.add_scalar('Spike-DDPG-NCP/raw_right_wheel_output', raw_snn_action[1], overall_steps)

            # Save Model
            if overall_steps % save_steps == 0:
                max_w, min_w, max_b, min_b, shape_w, shape_b = agent.save("../save_sddpgNCP_weights",
            ...
            if done or ita == ita_per_episode - 1:
                ...
                tb_writer.add_scalar('Spike-DDPG-NCP/avg_reward', episode_reward / (ita + 1), overall_steps)
                break
