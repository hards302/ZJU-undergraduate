...
sys.path.append('../../')
sys.path.append('..')
from training.train_spiking_ddpg.sddpgRSTDP_agent import RSTDPAgentSpiking
...
def train_sddpg(run_name="SNN_R1", exp_name="Rand_R1", episode_num=(100, 200, 300, 400),
    ...
    dirName = 'save_sddpgRSTDP_weights'
    try:
        os.mkdir('../' + dirName)
    ...
    print("Gazebo Env done...")
    agent = RSTDPAgentSpiking(state_num, action_num, spike_state_num,
    ...
    while True:
        ...
        for ita in range(ita_per_episode):
            ...
            # update with RSTDP ------------------------------------------------------------------
            agent._update(reward)

            # Train network with replay
            if len(agent.memory) > batch_size:
                actor_loss_value, critic_loss_value = agent.replay()
                tb_writer.add_scalar('Spike-DDPG-RSTDP/actor_loss', actor_loss_value, overall_steps)
                tb_writer.add_scalar('Spike-DDPG-RSTDP/critic_loss', critic_loss_value, overall_steps)
            ita_time_end = time.time()
            tb_writer.add_scalar('Spike-DDPG-RSTDP/ita_time', ita_time_end - ita_time_start, overall_steps)
            tb_writer.add_scalar('Spike-DDPG-RSTDP/action_epsilon', agent.epsilon, overall_steps)
            tb_writer.add_scalar('Spike-DDPG-RSTDP/raw_left_wheel_output', raw_snn_action[0], overall_steps)
            tb_writer.add_scalar('Spike-DDPG-RSTDP/raw_right_wheel_output', raw_snn_action[1], overall_steps)

            # Save Model
            if overall_steps % save_steps == 0:
                max_w, min_w, max_b, min_b, shape_w, shape_b = agent.save("../save_sddpgRSTDP_weights",
            ...
            if done or ita == ita_per_episode - 1:
                ...
                tb_writer.add_scalar('Spike-DDPG-RSTDP/avg_reward', episode_reward / (ita + 1), overall_steps)
                break
